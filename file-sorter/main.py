...
def reset_folder():
    for folder in sorter.folders:
        ...
        remover.undo_folders(sorter.sorted_folder, sorter.sort_type, sorter.folders)
...
sort4 = tk.Label(frame1, text="Containing folder (Default)", font=small_font, cursor=cursor)
sort4.place(relwidth=0.6, relheight=0.1, relx=0.375, rely=0.025)
sort4.bind("<Button-1>", choose_folder)

#BUTTONS AND DROPDOWN
choose_sort_type_drop = ttk.OptionMenu(frame1, sort_var, *sort_menu, command=choose_sort_type)
choose_sort_type_drop.place(relx=0.225, rely=0.15, relwidth=0.4, relheight=0.1)

sort_files_button = ttk.Button(frame1, text="Sort files", cursor=cursor, command=final_sort)
sort_files_button.place(relwidth=0.2, relheight=0.1, relx=0.675, rely=0.15)

undo_folders_button = ttk.Button(frame1, text="Reset folder", cursor=cursor, command=reset_folder)
