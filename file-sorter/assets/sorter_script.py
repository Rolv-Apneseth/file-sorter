...
class Sorter(object):
    ...
    def date_year_selection(self):
        ...
        year_entry_button = ttk.Button(
            date_pop_up,
            text="Submit",
            command=lambda: validate_and_set_year(year_entry.get()),
        )
    ...
    def select_sort_type(self):
        ...
        # pop_up_buttons
        date_sort = ttk.Button(
            self.sort_type_pop_up,
            text="Sort by date",
        ...
        file_type_sort = ttk.Button(
            self.sort_type_pop_up,
            text="Sort by file type",
            command=lambda: set_to_type_and_destroy(self),
        )
    ...
    def make_date_folders(self):
        ...
        self.folders = [
            str(year) for year in range(int(self.earliest), int(self.YEAR) + 1)
        ]
        ...
        for filename in os.listdir(self.sorted_folder):
            if not filename in self.folders:
                ...
                if int(lst[4]) < int(self.earliest):
                    print(
                        f"{filename} was last modified in {str(lst[4])} and a folder for that year does not exist, so the file was skipped during the sort"
                    )
                    continue
